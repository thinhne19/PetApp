...
export default function DictCategory() {
  ...
  const { categoryName } = params; // Lấy categoryName từ tham số truyền vào
  ...
  useEffect(() => {
    const fetchPets = async () => {
      try {
        const q = query(
          collection(db, "PetDictionary"),
          where("categoryId", "==", categoryName) // Tìm kiếm theo categoryId
        );
  ...
  const filteredPets = pets.filter((pet) =>
    removeDiacritics(pet.name).includes(removeDiacritics(searchQuery))
  );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.carouselContainer}>
        <Carousel
          ...
          data={pets.slice(0, 5)} // Hiển thị 5 thú cưng nổi bật
